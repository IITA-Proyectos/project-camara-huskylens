...
motorx = Motor("B")
motory = Motor("A")
# Esta información es muy importante que la edites.
# hub.port.F | Conexion del puerto con el lego Hub
# baudrate | Esto depende de la configuración de tu camara te recomiendo 9600 que es la más estable.


huskyLens = HuskyLensCamera(hub.port.F, baudrate = 9600, debug= False)

# Esta parte te indicara si la camara inica correctamente, con un emotico por encima del hub.
...
huskyLens.setCustomName("FollowGirl", 1)
...
while True:
    # Aqui se debe crear una variable y llamar a los bloques solicitando por su ID propio.
    cara1 = huskyLens.getBlocksByID(1)

    # Len se encarga de hacer la busqueda de la cantidad de objetos que aparecen con la misma ID.
    # == 1, basicamente dice que si encuentra esa cara aprendida va a hacer una acción en particular. 
    if len(cara1) == 1:
        display.show_image("ANGRY")
        
        x_position = cara1[0].x # Obtención eje X del objeto Cara1
        y_position = cara1[0].y # Obtención eje y del objeto Cara1
        
        if x_position < 140:  # Si el objeto está a la izquierda del eje X 
            motorx.run_for_degrees(-5, 100)
        elif x_position > 180:  # Si el objeto está a la derecha del eje X
            motorx.run_for_degrees(5, 100)
        elif y_position < 140: # Revisar  
            print("Arriba")
            motory.run_for_degrees(5,100) 
        elif y_position > 190: # Revisar
            print("Abajo")
            motory.run_for_degrees(-5, 100)
        else:
            motorx.stop()
    ...
    # En caso de que no encuentre ninguna cara aprendida dentro del sistema devolvera lo siguiente:
    else: 
